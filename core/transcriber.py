...
from utils.file_utils import get_output_path, create_temp_file, is_supported_file
...
class Transcriber:
    ...
    def extract_audio(self, video_path, audio_out=None):
        ...
        log_process_start(logger, "extract_audio", 
        ...
        try:
            ...
            clip.audio.write_audiofile(audio_out, codec='pcm_s16le', logger=None)
            clip.close()
    ...
    def transcribe(self, input_path, output_path=None):
        ...
        if output_path is None:
            ...
            output_path = get_output_path(input_path, f"_transcricao_{timestamp}", ".txt")
        
        log_process_start(logger, "transcribe", 
        ...
        try:
            ...
            if is_supported_file(input_path, 'video'):
                logger.info("Arquivo de entrada é um vídeo, extraindo áudio...")
                ...
                raise ValueError(f"Formato de arquivo não suportado: {input_path}")
            ...
            logger.error(f"Erro durante a transcrição: {str(e)}", exc_info=True)
            raise
