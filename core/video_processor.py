...
class VideoProcessor:
    ...
    def __init__(self, silence_thresh=None, min_silence_len=None, padding=None):
        """
        ...
        """
        self.silence_thresh = silence_thresh or VIDEO_SETTINGS['silence_threshold']
        self.min_silence_len = min_silence_len or VIDEO_SETTINGS['min_silence_len']
    ...
    def remove_silence(self, video_input_path, video_output_path=None):
        ...
        log_process_start(logger, "remover_silencios", 
        ...
        try:
            ...
            logger.info(f"Parâmetros do vídeo original: FPS={fps}, Resolução={resolution}, Duração={duration:.2f}s")
            ...
            silent_ranges = [(start - self.padding, end + self.padding) for start, end in silent_ranges]
            silent_ranges = [(max(0, start), min(len(audio_segment), end)) for start, end in silent_ranges]
            ...
            for start, end in audio_ranges:
                ...
                logger.info(f"Corte mantido: Início={start / 1000.0:.2f}s, Fim={end / 1000.0:.2f}s, Duração={duration:.2f}s")
            
            # Cria os clipes de vídeo correspondentes aos segmentos com áudio
            logger.info("Criando clipes de vídeo correspondentes aos segmentos com áudio...")
            clips = [video.subclipped(start / 1000.0, end / 1000.0) for start, end in audio_ranges]
