...
class MainApplication(tk.Tk):
    ...
    def _on_close(self):
        ...
        if messagebox.askokcancel(
            'Sair', 'Deseja realmente sair da aplicação?'
        ):
            self.destroy()

    def show(self):
        """Exibe a aplicação e inicia o loop de eventos."""
        self.mainloop()
...
class VideoEditorTab(ProcessingTab):
    ...
    def _create_widgets(self):
        ...
        options_inner_frame = ttk.Frame(
            self.options_frame, style='Card.TFrame'
        )
        options_inner_frame.pack(fill='x', padx=10, pady=10, expand=True)

        # Slider para limiar de silêncio
        ttk.Label(options_inner_frame, text='Limiar de silêncio (dB):').grid(
            row=0, column=0, padx=10, pady=10, sticky='w'
        )
        self.silence_threshold = tk.IntVar(
            value=VIDEO_SETTINGS['silence_threshold']
        )
        ...
        self.silence_threshold_label = ttk.Label(
            options_inner_frame, text=str(self.silence_threshold.get())
        )
        self.silence_threshold_label.grid(row=0, column=2, padx=5, pady=10)
        self.threshold_scale.configure(
            command=lambda v: self.silence_threshold_label.configure(
        ...
        ttk.Label(
            options_inner_frame, text='Duração mínima silêncio (ms):'
        ).grid(row=1, column=0, padx=10, pady=10, sticky='w')
        self.min_silence_len = tk.IntVar(
            value=VIDEO_SETTINGS['min_silence_len']
        )
        ...
        self.min_silence_scale.grid(
            row=1, column=1, padx=10, pady=10, sticky='we'
        )
        self.min_silence_label = ttk.Label(
            options_inner_frame, text=str(self.min_silence_len.get())
        )
        self.min_silence_label.grid(row=1, column=2, padx=5, pady=10)
        self.min_silence_scale.configure(
            command=lambda v: self.min_silence_label.configure(
                text=str(int(float(v)))
            )
        )

        # Slider para padding
        ttk.Label(options_inner_frame, text='Padding (ms):').grid(
        ...
        self.padding_label = ttk.Label(
            options_inner_frame, text=str(self.padding.get())
        )
        self.padding_label.grid(row=2, column=2, padx=5, pady=10)
        self.padding_scale.configure(
            command=lambda v: self.padding_label.configure(
                text=str(int(float(v)))
            )
        )
    ...
    def _process_video(self):
        ...
        if not os.path.exists(video_path):
            messagebox.showerror(
                'Erro', f'Arquivo não encontrado: {video_path}'
            )
            return
        ...
        threading.Thread(
            target=self._run_video_processing, args=(video_path,), daemon=True
        ).start()
...
class FrameExtractorTab(ProcessingTab):
    ...
    def _create_widgets(self):
        ...
        # Opções de extração
        ttk.Label(self.options_frame, text='Método de extração:').grid(
        ...
        ttk.Radiobutton(
            self.options_frame,
            text='Frames regulares',
            variable=self.extraction_method,
            value='regulares',
        ).grid(row=0, column=2, padx=5, pady=5, sticky='w')
        ttk.Radiobutton(
            self.options_frame,
            text='Diferença entre quadros',
            variable=self.extraction_method,
            value='diferenca',
        ...
        ttk.Label(self.adv_options_frame, text='Threshold:').grid(
            row=0, column=0, padx=5, pady=5, sticky='w'
        )
        ...
        ttk.Label(self.adv_options_frame, text='Intervalo de frames:').grid(
        ...
        ttk.Checkbutton(
            self.adv_options_frame,
            text='Usar ThresholdDetector',
            variable=self.use_threshold_detector,
    ...
    def _extract_frames(self):
        ...
        # Iniciar o processamento em uma thread separada
        threading.Thread(
            target=self._run_frame_extraction, args=(video_path,), daemon=True
        ).start()
        ...
        try:
            ...
            if method == 'cenas':
                ...
                output_dir = extractor.extract_regular_frames(
                    video_path, intervalo=self.interval.get()
                )
    ...
    def _extraction_finished(self, output_dir):
        ...
        messagebox.showinfo(
            'Concluído',
            f'Frames extraídos com sucesso!\nSalvos em:\n{output_dir}',
        )
    ...
    def _processing_error(self, error_message):
        ...
        self.set_status(f'Erro: {error_message}')
        messagebox.showerror(
            'Erro', f'Ocorreu um erro durante a extração:\n{error_message}'
        )
...
class ImageUpscalerTab(ProcessingTab):
    ...
    def _create_widgets(self):
        ...
        self.dir_selector = DirectorySelector(
            self.selector_container, title='Selecione a pasta com imagens'
        )

        # Opções de upscaling
        ttk.Label(self.options_frame, text='Fator de escala:').grid(
            row=0, column=0, padx=5, pady=5, sticky='w'
        )
    ...
    def _process_images(self):
        ...
        # Iniciar o processamento em uma thread separada
        threading.Thread(
            target=self._run_image_processing, args=(path, mode), daemon=True
        ).start()
    ...
    def _run_image_processing(self, path, mode):
        ...
        try:
            ...
            enhancer = ImageEnhancer(scale=self.scale_factor.get())
    ...
    def _processing_finished_dir(self, output_dir):
        ...
        messagebox.showinfo(
            'Concluído',
            f'Todas as imagens foram processadas!\nSalvas em:\n{output_dir}',
        )
...
class TranscriptionTab(ProcessingTab):
    ...
    def _create_widgets(self):
        ...
        ttk.Label(self, text='Transcrição:').pack(
        ...
        self.transcription_text = tk.Text(
            self.transcription_frame, wrap='word', height=10
        )
        self.transcription_text.pack(fill='both', expand=True, side='left')

        scrollbar = ttk.Scrollbar(
            self.transcription_frame, command=self.transcription_text.yview
        )
    ...
    def _transcribe_file(self):
        ...
        if not file_path:
            messagebox.showerror(
                'Erro', 'Selecione um arquivo de áudio ou vídeo.'
            )
            return
        ...
        # Iniciar o processamento em uma thread separada
        threading.Thread(
            target=self._run_transcription, args=(file_path,), daemon=True
        ).start()
    ...
    def _run_transcription(self, file_path):
        ...
        try:
            ...
            transcriber = Transcriber(
                model_name=self.model.get(), language=self.language.get()
            )
            ...
            # Atualizar a UI na thread principal
            self.after(
                0, self._transcription_finished, output_path, srt_path, text
            )

        except Exception as e:
            # Tratar erros
            logger.error(
                f'Erro ao transcrever arquivo: {str(e)}', exc_info=True
            )
            self.after(0, self._processing_error, str(e))
    ...
    def _processing_error(self, error_message):
        ...
        self.set_status(f'Erro: {error_message}')
        messagebox.showerror(
            'Erro', f'Ocorreu um erro durante a transcrição:\n{error_message}'
        )
